...
def upvote_comment(request, comment_id):
    if (not hasattr(request.user, 'userprofile') or
       not request.user.userprofile.is_member()):
        request.session['error_message'] = ('You must be logged in and '
                                            'a member to vote')
        return {
            'fragments': {
                '#ajax-message': r'''
    <div id="ajax-message" class="alert alert-danger">
    ...
    existing_downvotes = MessagePoint.objects.filter(
                                    user=profile,
                                    message=message,
                                    plus_point=False
    )
    ...
    return {
            'fragments': {
                    '#upvote'+comment_id: r'''
                    <a id="upvote%s" class="btn btn-warning" onclick="$('#downvote%s').attr('disabled',true);$('#upvote%s').attr('disabled',true);ajaxGet('%s',function(){$('#upvote%s').attr('disabled',false);$('#downvote%s').attr('disabled',false);})">
                    Withdraw upvote
                    </a>
                    ''' % (
                        comment_id,
                    ...
                    '#downvote'+comment_id: r'''
                    <a id="downvote%s" class="btn btn-primary" onclick="$('#downvote%s').attr('disabled',true);$('#upvote%s').attr('disabled',true);ajaxGet('%s',function(){$('#upvote%s').attr('disabled',false);$('#downvote%s').attr('disabled',false);})">
                    Switch to downvote
                    </a>
                    ''' % (
                        comment_id,
                        comment_id,
                        comment_id,
                        reverse('fora:downvote_comment', args=[comment_id]),
                        comment_id,
                        comment_id
                    ),
                    '#points'+comment_id: r'''
                    <span id="points11">%d</span>
                    ''' % (message.get_net_points()),
            }
    }
...
def withdraw_upvote(request, comment_id):
    ...
    message = get_object_or_404(ForumMessage, id=comment_id)
    profile = request.user.userprofile.memberprofile
    existing_upvotes = MessagePoint.objects.filter(
                            user=profile,
                            message=message,
                            plus_point=True
    )
    ...
    return {
        'fragments': {
            '#upvote'+comment_id: r'''
            <a id="upvote%s" class="btn btn-success" onclick="$('#downvote%s').attr('disabled',true);$('#upvote%s').attr('disabled',true);ajaxGet('%s',function(){$('#upvote%s').attr('disabled',false);$('#downvote%s').attr('disabled',false);})">
            Upvote
            </a>
            ''' % (
                comment_id,
                comment_id,
                comment_id,
                reverse('fora:upvote_comment', args=[comment_id]),
                comment_id,
                comment_id
            ),
            '#downvote'+comment_id: r'''
            <a id="downvote%s" class="btn btn-danger" onclick="$('#downvote%s').attr('disabled',true);$('#upvote%s').attr('disabled',true);ajaxGet('%s',function(){$('#upvote%s').attr('disabled',false);$('#downvote%s').attr('disabled',false);})">
            Downvote
            </a>
            ''' % (
                comment_id,
...
def withdraw_downvote(request, comment_id):
    ...
    return {
        'fragments': {
            ...
            '#points'+comment_id: r'''
            <span id="points11">%d</span>''' % (message.get_net_points()),
        }
    }
...
def downvote_comment(request, comment_id):
    ...
    if get_user_points(profile) <= 0:
        request.session['error_message'] = ('Downvoting costs 1 point. You '
                                            'lack sufficient points.')
        return {
            'fragments': {
                '#ajax-message': r'''
    <div id="ajax-message" class="alert alert-danger">
    ...
    if profile in message.get_downvoters():
        request.session['error_message'] = ('You have already downvoted '
                                            'this post')
        return {
            'fragments': {
                '#ajax-message': r'''
    ...
    return {
        'fragments': {
            ...
            Withdraw downvote
                ...
                comment_id,
                comment_id,
                reverse('fora:withdraw_downvote', args=[comment_id]),
                comment_id,
                comment_id
            ),
            '#upvote'+comment_id: r'''
...
def get_thread_page(request, forum_id, page_num):
    ...
    if page_num > 0:
        down_options = r'''onclick="ajaxGet('%s',function(){})"''' % (
                            reverse('fora:get_thread_page', args=[
                                                                forum_id,
                                                                page_num-1
                            ])
        )
    ...
    if page_num < (max_pages-1):
        up_options = r'''onclick="ajaxGet('%s',function(){})"''' % (
                            reverse('fora:get_thread_page',
                                    args=[
                                        forum_id,
                                        page_num+1
                                    ])
    ...
    context_dict.update(get_permissions(request.user))
    context_dict.update(get_common_context(request))
    thread_list_html = loader.render_to_string(
                                'fora/thread_list.html',
                                context_dict
    )
    return {
        'fragments': {
            '#forum_pager'+forum_id: r'''
        <ul class="pagination" id="forum_pager%s">
