...
class AboutViewsTestCase(TestCase):
    ...
    def test_views_joining(self):
        ...
        for officer in officers:
            ...
            self.assertTrue('can_edit_page' in context)
            position_name = unicode(officer.position)
            if officer.position.name in self.joining_list:
                self.assertTrue(context['can_edit_page'],
                                msg='%s unable to edit' % position_name)
            else:
                self.assertFalse(context['can_edit_page'],
                                 msg='%s able to edit' % position_name)
            client.logout()
    ...
    def test_views_leadership(self):
        ...
        self.assertEqual(resp.context['requested_term'],
        ...
        old_term = AcademicTerm.objects.get(year=2011,
                                            semester_type__name='Fall')
        resp = self.client.get(reverse('about:leadership_for_term',
                                       args=[old_term.id]))
        self.assertEqual(resp.status_code, 200)
        self.assertTrue('officers' in resp.context)
        self.assertTrue('officers' in resp.context['officers'])
        ...
        self.assertEqual(resp.context['terms'].count(), 5)
        self.assertTrue('requested_term' in resp.context)
        self.assertEqual(resp.context['requested_term'], old_term)
        self.assertTrue('is_current' in resp.context)
        ...
        # further testing should be done in pack_officers method
    ...
    def test_update_photos(self):
        ...
        self.assertEqual(AboutSlideShowPhoto.objects.count(), 1)
        self.assertFormsetError(resp, 'formset', 1, 'title',
                                'This field is required.')
        self.assertFormsetError(resp, 'formset', 1, 'text',
                                'This field is required.')
        self.assertFormsetError(resp, 'formset', 1, 'photo',
                                'This field is required.')
        ...
        self.assertFalse(resp.context['success_message'])
        self.assertEqual(AboutSlideShowPhoto.objects.count(), 1)
        self.assertFormsetError(resp, 'formset', 0, 'title',
                                'This field is required.')
        ...
        self.assertFalse(resp.context['success_message'])
        self.assertEqual(AboutSlideShowPhoto.objects.count(), 1)
        invalid_image_error = ('Upload a valid image. The file you uploaded '
    ...
    def test_update_joining(self):
        ...
        self.assertFormsetError(resp, 'formset', 2, 'section',
                                'This field is required.')
        self.assertFormsetError(resp, 'formset', 2, 'text',
                                'This field is required.')
        client.logout()
