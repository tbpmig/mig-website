...
This is an automated notice that a member, %(member_name)s (%(uniqname)s), has
updated their (alumni) info on the website. Their new info is:
...
This is an automated notice that a member, %(member_name)s (%(uniqname)s), has
updated their (alumni) info on the website. They were previously considered an
alum, but now are not.
...
The TBP Website''' % {'member_name': profile.get_firstlast_name(),
    ...
    send_mail('[TBP] Notice of alumni profile update.',
              body,
              'tbp.mi.g@gmail.com',
              [mem.email],
              fail_silently=False)
...
def get_electees_with_status(distinction):
    ...
    electee_profiles = MemberProfile.get_electees().filter(
                                                standing=dist_standing)
    electees_with_status = []
    for profile in electee_profiles:
        packaged_progress = ProgressItem.package_progress(
                                ProgressItem.objects.filter(
                                        member=profile,
        ...
        amount_req = 0
        amount_has = 0
        has_dist = distinction.has_distinction_met(
                                    packaged_progress,
                                    unflattened_reqs)
        if has_dist:
...
def get_electees_who_completed_reqs():
    ugrad_distinction = DistinctionType.objects.get(
                            status_type__name="Electee",
                            standing_type__name="Undergraduate",
                            name='Electee (undergrad)'
    )
    grad_distinction = DistinctionType.objects.get(
                            status_type__name="Electee",
                            standing_type__name="Graduate",
                            name='Electee (grad)'
    )
    ...
    ugrad_profiles = MemberProfile.get_electees().filter(
                        standing__name='Undergraduate').order_by('last_name')
    grad_profiles = MemberProfile.get_electees().filter(
                        standing__name='Graduate').order_by('last_name')
    electees_with_status = []
    for profile in ugrad_profiles:
        ...
        has_dist = ugrad_distinction.has_distinction_met(
                                            packaged_progress,
                                            unflattened_ugrad_reqs
        )
        if has_dist:
            electees_with_status.append(profile)
    for profile in grad_profiles:
        ...
        has_dist = grad_distinction.has_distinction_met(
                                        packaged_progress,
                                        unflattened_grad_reqs
        )
        if has_dist:
            electees_with_status.append(profile)
    return electees_with_status
...
def get_events_signed_up_hours(uniqname):
    events_attending = CalendarEvent.objects.filter(
                            eventshift__attendees__uniqname=uniqname
                        ).filter(completed=False).distinct()
    category_hours = {}
    for event in events_attending:
        if ProgressItem.objects.filter(
                            member__uniqname=uniqname,
                            related_event=event):
            continue
        if event.event_type not in category_hours:
            category_hours[event.event_type] = 0
        shifts = event.eventshift_set.filter(
                        attendees__uniqname=uniqname).order_by('start_time')
        n = shifts.count()
        count = 0
        hours = 0
        while count < n:
...
def package_future_progress(packaged_progress, category_hours):
    ...
    for event_type, hours in category_hours.items():
        ...
        while event_type_temp.parent_category is not None:
...
def sorted_reqs2html_recursive(sorted_reqs,
                               progress_items,
                               distinctions,
                               padding):
    html_string = ''
    for event_category, data in sorted_reqs.items():
        ...
        for distinction in distinctions.order_by('display_order'):
            ...
            html_string += '</td>\n'
        html_string += '</tr>\n'
        html_string += sorted_reqs2html_recursive(
                                    data["children"],
                                    progress_items,
                                    distinctions,
                                    padding+2)
...
def sorted_reqs2html(sorted_reqs, progress_items, distinctions):
    ...
    html_string += '</tr>\n</thead>'
    return html_string+sorted_reqs2html_recursive(sorted_reqs,
                                                  progress_items,
                                                  distinctions,
                                                  0) + '</table>'
...
def index(request):
    ...
    if user_is_member(request.user) and 'error' not in request.session:
...
def member_profiles(request):
    ...
    if not user_is_member(request.user):
        request.session['error_message'] = ('You must be logged in and a '
                                            'member to view member profiles.')
    ...
    if not active_html:
        print 'blah'
        print MemberProfile.get_actives().count()
...
def profile(request, uniqname):
    ...
    distinctions = DistinctionType.objects.filter(
                                    status_type=profile.status,
                                    standing_type=profile.standing
                            ).distinct()
    distinction_terms = []
    has_distinctions = False
    for distinction in distinctions:
        terms = Distinction.objects.filter(member=profile,
    ...
    context_dict = {
        'profile': profile,
        'officer_positions': sorted(
                                officer_positions,
                                key=lambda k: (-k['term'].year,
                                               k['term'].semester_type.name)
        ),
...
def profile_edit(request, uniqname):
    ...
    context_dict = {
        ...
        'back_button': {
                    'link': reverse('member_resources:profile', args=[uniqname]),
                    'text': 'To %s\'s Profile' % (profile.get_firstlast_name())
        },
        'form_title': 'Edit %s\'s Profile' % (unicode(profile)),
        'help_text': ('Make updates to your profile here. Of special note, '
...
def profile_create(request):
    ...
    context_dict = {
        ...
        'help_text': ('Make your profile here. Of special note, resumes must '
...
def edit_progress(request, uniqname):
    ...
    if profile.status.name == 'Active':
        ...
        if not Permissions.can_manage_electee_progress(request.user):
            ...
            return redirect('member_resources:index')
    term = AcademicTerm.get_current_term()
    progress_formset = modelformset_factory(ProgressItem,
                                            exclude=('member', 'term'),
                                            can_delete=True)
    formset = progress_formset(
    ...
    context_dict = {
        ...
        'back_button': {
                    'link': reverse('member_resources:view_progress',
                                    args=[uniqname]),
                    'text': 'To %s\'s Progress' % (profile.get_firstlast_name())
        },
...
def view_progress(request, uniqname):
    ...
    distinctions = DistinctionType.objects.filter(
                                    status_type=profile.status,
                                    standing_type=profile.standing).distinct()
    if profile.status.name == 'Active':
    ...
    progress = ProgressItem.objects.filter(member=profile,
                                           term=AcademicTerm.get_current_term())
...
def download_active_progress(request):
    ...
    for profile in active_profiles:
        ...
        row = {
            'member': profile,
            'progress': flatten_progress(packaged_progress, active_reqs)
        }
        dist_progress = []
...
def download_grad_el_progress(request):
    ...
    writer = UnicodeWriter(response)
    distinctions_grad_el = DistinctionType.objects.filter(
                                status_type__name="Electee").filter(
                                    standing_type__name="Graduate").distinct()
    query = Q()
    ...
    grad_el_profiles = Permissions.profiles_you_can_view(
                            request.user).filter(
                                status__name="Electee").filter(
                                        standing__name="Graduate")
    for profile in grad_el_profiles:
        ...
        row = {
            'member': profile,
            'progress': flatten_progress(packaged_progress, grad_electees_reqs)
        }
        dist_progress = []
        for distinction in distinctions_grad_el.order_by('name'):
            ...
            for event_category, data in unflattened_reqs.items():
                ...
                if amount_has_temp > amount_req_temp:
                    ...
                    amount_has = amount_has + amount_has_temp
                amount_req = amount_req+amount_req_temp
            has_dist = distinction.has_distinction_met(packaged_progress,
                                                       unflattened_reqs)
            close_dist = (amount_has/amount_req)>.75
            dist_progress.append(unicode(has_dist))
            dist_progress.append(unicode(close_dist))
...
def download_ugrad_el_progress(request):
    ...
    writer = UnicodeWriter(response)
    distinctions_ugrad_el = DistinctionType.objects.filter(
                                status_type__name='Electee').filter(
                                    standing_type__name='Undergraduate'
                                        ).distinct()
    query = Q()
    ...
    ugrad_el_profiles = Permissions.profiles_you_can_view(
                            request.user).filter(
                                status__name='Electee').filter(
                                    standing__name='Undergraduate')
    for profile in ugrad_el_profiles:
        ...
        row = {
            'member': profile,
            'progress': flatten_progress(packaged_progress, ugrad_electees_reqs)
        }
        dist_progress = []
...
def assemble_table(user, keys, status_name, standing_names):
    ...
    if not distinctions:
        distinctions = DistinctionType.objects.filter(
                            status_type__name=status_name).filter(
                                standing_type__name__in=standing_names
    ...
    if not reqs or not progress_rows:
        ...
        progress_rows = []
        profiles = Permissions.profiles_you_can_view(user).filter(
                            status__name=status_name).filter(
                                standing__name__in=standing_names)
        for profile in profiles:
            ...
            row = {
                'member': profile,
                'progress': flatten_progress(packaged_progress, reqs),
                'future_progress': flatten_progress(
                                        packaged_future_progress,
                                        reqs
                )
            }
            merged_progress = []
            for count in range(len(row['progress'])):
                merged_progress.append({
                                        'has': row['progress'][count],
                                        'will_have': row['future_progress'][count]
                                        })
...
def view_progress_table(request):
    ...
    if can_manage_actives:
        distinctions_actives, active_reqs, progress_rows = assemble_table(
                                            request.user,
                                            (
                                                'PROGRESS_TABLE_ACTIVE_DIST',
                                                'PROGRESS_TABLE_ACTIVE_REQS',
                                                'PROGRESS_TABLE_ACTIVE_ROWS'
                                            ),
                                            'Active',
                                            [
                                                'Undergraduate',
    ...
    if can_manage_electees:
        distinctions_ugrad_el, ugrad_electees_reqs, progress_rows_ugrad_el = assemble_table(
                                                        request.user,
                                                        (
                                                            'PROGRESS_TABLE_UGRADEL_DIST',
                                                            'PROGRESS_TABLE_UGRADEL_REQS',
                                                            'PROGRESS_TABLE_UGRADEL_ROWS'
                                                        ),
                                                        'Electee',
                                                        ['Undergraduate']
        )
        distinctions_grad_el, grad_electees_reqs, progress_rows_grad_el = assemble_table(
                                                        request.user,
                                                        (
                                                            'PROGRESS_TABLE_GRADEL_DIST',
                                                            'PROGRESS_TABLE_GRADEL_REQS',
                                                            'PROGRESS_TABLE_GRADEL_ROWS'
                                                        ),
                                                        'Electee',
                                                        ['Graduate']
        )
...
def upload_minutes(request):
    if not Permissions.can_upload_minutes(request.user):
        request.session['error_message'] = ('You are not authorized to upload '
                                            'minutes.')
        return redirect('member_resources:index')
    MeetingMinutesForm.base_fields['display_order'].initial = MeetingMinutes.get_next_meeting_minutes_display_order()
    form = MeetingMinutesForm(request.POST or None, request.FILES or None)
    if request.method == 'POST':

        if form.is_valid():
            form.save()
            request.session['success_message'] = 'Minutes upload successful'
            return get_previous_page(
                                request,
                                'member_resources:officer_meeting_minutes')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'back_button': {
                    'link': reverse('member_resources:main_meeting_minutes'),
                    'text': 'To Meeting Minutes'
        },
...
def project_reports_list(request):
    ...
    if tmp_user.is_superuser:
        ...
        current_positions = get_officer_positions_predecessors(
                Permissions.get_current_officer_positions_positions(tmp_user))
        ...
        for officer in current_positions:
            ...
            if limit_term < AcademicTerm.objects.get(
                                semester_type__name='Winter',
                                year=2013):
                # I think this is since they were only compiled yearly prior
                # to that?
                limit_term = limit_term.get_next_full_term()
            old_reports |= CompiledProjectReport.objects.filter(
                                is_full=False,
                                associated_officer=officer,
                                term__lte=limit_term
            )
...
def view_misc_reqs(request):
    if not (Permissions.can_change_requirements(request.user) or
            Permissions.can_manage_misc_reqs(request.user)):
    ...
    context_dict = {
        'can_add_electee_members': Permissions.can_add_electee_members(
                                                                request.user),
        'can_manage_project_leaders': Permissions.can_manage_project_leaders(
                                                                request.user),
        'can_manage_officers': Permissions.can_manage_officers(request.user),
        'can_manage_committees': Permissions.can_manage_committees(
                                                                request.user),
        'terms': AcademicTerm.objects.all().exclude(
                                semester_type__name='Summer').exclude(
                                    id=current_term.id).exclude(
                                        id=next_term.id).order_by(
                                                            '-year',
                                                            '-semester_type'),
        ...
        'can_manage_interviews': (
                    Permissions.can_manage_active_progress(request.user) or
                    Permissions.can_manage_electee_progress(request.user)
        ),
        ...
        'grad_electee_distinctions': DistinctionType.objects.filter(
                                                status_type__name='Electee',
                                                standing_type__name='Graduate'
        ),
        'subnav': 'misc_reqs',
...
def manage_website(request):
    ...
    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            request.session['success_message'] = ('Home Page Slideshow '
...
def advance_term(request):
    ...
    if current_terms.count() != 1:
        raise IntegrityError(('There must be one and only one current term, '
                              'inspect the database as there are '
                              '%d current terms') % (current_terms.count()))
    term = AcademicTerm.get_current_term()
    next_term = term.get_next_term()
    c = current_terms[0]
...
def manage_officers(request, term_id):
    ...
    formset = ManageOfficersFormSet(request.POST or None,
                                    request.FILES or None,
                                    prefix='officers',
                                    queryset=Officer.objects.filter(term=term))
    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            for instance in formset.new_objects:
                instance.term = [term]
                instance.save()
            request.session['success_message'] = ('Officers updated '
                                                  'successfully')
    ...
    context_dict = {
        ...
        'form_title': 'Add/Remove Members as Officers for %s' % (
                                                        unicode(term)),
        'help_text': ('Add or update the bios and pictures for officers. This '
                      'will affect their permissions for the given term.'),
        'back_button': {
...
def manage_committee_members(request, term_id):
    if not Permissions.can_manage_committees(request.user):
        ...
        return redirect('member_resources:index')
    ManageCommitteesFormSet = modelformset_factory(
                                        CommitteeMember,
    ...
    term = get_object_or_404(AcademicTerm, id=term_id)
    prefix = 'committees'
    formset = ManageCommitteesFormSet(
                            request.POST or None,
                            prefix=prefix,
                            queryset=CommitteeMember.objects.filter(term=term)
    )
    if request.method == 'POST':
        if formset.is_valid():
            instances = formset.save(commit=False)
            ...
            for instance in formset.deleted_objects:
                instance.delete()
            request.session['success_message'] = ('Committee Members updated '
                                                  'successfully')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'form_title': 'Add/Remove members as committee members for %s' % (
                                                                unicode(term)),
        'help_text': ('Add or update the committee members for the given term.'
                      ' This will cause them to display on the leadership '
                      'page.'),
        'back_button': {
                'link': reverse('member_resources:view_misc_reqs'),
                'text': 'To Membership Management'
...
def manage_committees(request):
    if not Permissions.can_manage_officers(request.user):
        request.session['error_message'] = ('You are not authorized to manage '
                                            'chapter committees.')
        return redirect('member_resources:index')
    ManageCommitteesFormSet = modelformset_factory(Committee,exclude=[])
    prefix = 'committees'
    formset = ManageCommitteesFormSet(
                        request.POST or None,
                        prefix=prefix,
    ...
    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            request.session['success_message'] = ('Committees updated '
                                                  'successfully')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'help_text': ('Add or update the committees for the chapter. This will'
                      ' cause them to display on the leadership (for each that'
...
def add_electee_DA_PA_status(request):
    if not Permissions.can_manage_active_progress(request.user):
        request.session['error_message'] = ('You are not authorized to manage '
                                            'electee DA/PA status.')
        return redirect('member_resources:index')
    term = AcademicTerm.get_current_term()
    if request.method == 'POST':
        formset = ManageElecteeDAPAFormSet(request.POST,
        ...
        if formset.is_valid():
            ...
            for inst in formset.new_objects:
                if inst:
                    inst.term = term
                    if 'DA' in inst.distinction_type.name:
                        inst.distinction_type = DistinctionType.objects.get(
                                            name='Distinguished Active'
                        )
                    elif 'PA' in inst.distinction_type.name:
                        inst.distinction_type = DistinctionType.objects.get(
                                            name='Prestigious Active'
                        )
                    if not Distinction.objects.filter(
                                    member=inst.member,
                                    distinction_type=inst.distinction_type,
                                    term=term).exists():
                        inst.save()
            request.session['success_message'] = ('Active Statuses '
                                                  'updated successfully')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
        ...
        for distinction in DistinctionType.objects.filter(
                                status_type__name='Electee').filter(
                                                    name__contains='DA'):
            related_distinction = DistinctionType.objects.get(
                                            name='Distinguished Active')
            electees_already_received_dist = MemberProfile.objects.filter(
                            distinction__distinction_type=related_distinction,
                            distinction__term=AcademicTerm.get_current_term()
            )
            electees = get_electees_with_status(distinction)
            for electee in electees:
                if electee in electees_already_received_dist:
                    continue
                gift = 'Not specified'
                initial.append({
                            'member': electee,
        ...
        for distinction in DistinctionType.objects.filter(
                    status_type__name='Electee').filter(name__contains='PA'):
            related_distinction = DistinctionType.objects.get(
                                                    name='Prestigious Active')
            electees_already_received_dist = MemberProfile.objects.filter(
                            distinction__distinction_type=related_distinction,
        ...
        formset = ManageElecteeDAPAFormSet(
                            queryset=Distinction.objects.none(),
                            initial=initial,
                            prefix='current_status'
        )
    template = loader.get_template('generic_formset.html')
    context_dict = {
        ...
        'form_title': 'Add Electee DA/PA Distinctions for  %s' % (
                                                                unicode(term)),
        'back_button': {
                'link': reverse('member_resources:view_misc_reqs'),
                'text': 'To Membership Management'
        },
        'help_text': ('This list is pre-populated with members who have enough'
                      ' credit on the website to receive the noted status. '
                      'Members whose status has already been logged are '
                      'omitted.'),
        }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
...
def add_active_statuses_for_term(request, term_id):
    ...
    context_dict = {
        ...
        'help_text': ('This list is pre-populated with members who have '
                      'enough credit on the website to receive the noted '
                      'status. Members whose status has already been logged '
                      'are omitted.'),
        }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
...
def manage_active_statuses(request):
    ...
    context_dict = {
        ...
        'help_text': ('This list contains members\' status that have already '
                      'been logged. You may edit or remove any status listed '
                      'here. You may also add new statuses, but if for the '
                      'current term, the \'Add Active/DA/PA Status \' tool is '
                      'recommended.'),
        }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
        ...
        if form.is_valid():
            ...
            for uniqname in uniqnames:
                ...
                if current_actives.filter(uniqname=uniqname_stripped).exists():
                    ...
                    continue
                if (current_electees_ugrad.filter(
                                    uniqname=uniqname_stripped).exists() or
                    current_electees_grad.filter(
                                    uniqname=uniqname_stripped).exists()):
                ...
                a.save()
            if not (error_lists['bad_uniqnames'] or
                    error_lists['current_actives'] or
                    error_lists['current_electees']):
                request.session['success_message'] = ('All uniqnames added '
                                                      'successfully')
                return redirect('member_resources:view_misc_reqs')
            else:
                request.session['warning_message'] = ('Some uniqnames were '
                                                      'not added')
                form = MassAddForm(
                        initial={
                            'uniqnames': '\n'.join(
                                        error_lists['bad_uniqnames'] +
                                        error_lists['current_actives'] +
                                        error_lists['current_electees']
                            )
                        },
                        prefix='mass-add'
        ...
        if form.is_valid():
            ...
            for uniqname in uniqnames:
                ...
                if not expr.match(uniqname_stripped):
                    error_lists['bad_uniqnames'].append(uniqname_stripped)
                    continue
                to_delete1 = current_electees_ugrad.filter(
                                        uniqname=uniqname_stripped)
                to_delete2 = current_electees_grad.filter(
                                        uniqname=uniqname_stripped)
                if not to_delete1.exists() and not to_delete2.exists():
                    ...
                    to_delete1.delete()
                    to_delete2.delete()
            if (not error_lists['bad_uniqnames'] and
                    not error_lists['missing_uniqnames']):
                request.session['success_message'] = ('All uniqnames removed '
                                                      'successfully')
                return redirect('member_resources:view_misc_reqs')
            else:
                request.session['warning_message'] = ('Some uniqnames were '
...
def bulk_add_leadership_credit(request):
            ...
            for uniqname in uniqnames:
                ...
                if members:
                    if ProgressItem.objects.filter(
                                        member=members[0],
                                        term=term,
                                        event_type=leadership_category
                                    ).exists():
                        continue
                    leadership_credit = ProgressItem(
                                    member=members[0],
                                    event_type=leadership_category,
                                    date_completed=date.today(),
                                    amount_completed=1,
                                    term=AcademicTerm.get_current_term(),
                                    name='Leadership Credit'
                    )
                    leadership_credit.save()
                else:
                    error_list.append(uniqname)
            if not error_list:
                request.session['success_message'] = ('Leadership credit added'
                                                      ' for all uniqnames')
                return redirect('member_resources:view_misc_reqs')
            else:
                request.session['warning_message'] = ('Not all leadership '
                                                      'credits added.')
                form = MassAddForm(
                            initial={
                                'uniqnames': '\n'.join(error_list)
    ...
    template = loader.get_template(
                        'member_resources/add_leadership_credit.html')
    context_dict = {
        'formset': None,
        'mass_form': form,
...
def add_leadership_credit(request):
    ...
    term = AcademicTerm.get_current_term()
    leadership_category = EventCategory.objects.get(name='Leadership')
    formset = LeadershipCreditFormSet(request.POST or None,
                                      prefix='leadership')
    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            request.session['success_message'] = ('Leadership credits added '
                                                  'successfully')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
...
def move_electees_to_active(request):
    if not Permissions.can_manage_electee_progress(request.user):
        request.session['error_message'] = ('You are not authorized to give '
                                            'electees active status.')
        return redirect('member_resources:index')
    error_list = []
    prefix = 'electee2active'
    # should probably add extra checking here to ensure added members have
    # correct distinction
    term = AcademicTerm.get_current_term()
    if request.method == 'POST':
        formset = ElecteeToActiveFormSet(request.POST, prefix=prefix)
        if formset.is_valid():
            instances = formset.save(commit=False)
            for obj in formset.deleted_objects:
                obj.delete()
            # technically should check here that the distinction doesn't
            # already exist...
            for instance in instances:
                if instance:
                    instance.term = term
            ...
            request.session['success_message'] = ('Selected electees '
        ...
        for electee in electees_with_reqs:
            stand_stat_query = Q(status_type__name='Electee',
                                 standing_type=electee.standing)
            DA_PA_q = Q(name__contains='DA') | Q(name__contains='PA')
            initial.append({
                        'member': electee,
                        'distinction_type': DistinctionType.objects.get(
                                                stand_stat_query & ~DA_PA_q
                        ),
                        'gift': 'None'}
            )
        ElecteeToActiveFormSet.extra = len(initial) + 1
        formset = ElecteeToActiveFormSet(
                                queryset=Distinction.objects.none(),
                                initial=initial,
                                prefix=prefix
        )
    ...
    context_dict = {
        ...
        'help_text': ('To note that an electee has completed their '
                      'requirements and been initiated, check the approve box '
                      'next to his/her name and then click the button at the '
                      'bottom. This will move the electee from being '
                      'considered an electee to being considered an active. '
                      'NOTE: If the electee also completed electee DA/PA '
                      'status you\'ll need to take note of this as those '
                      'statuses are only show up for electees.'),
    }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
    return HttpResponse(template.render(context_dict, request))
...
def manage_project_leaders(request):
    if not Permissions.can_manage_project_leaders(request.user):
        request.session['error_message'] = ('You are not authorized to manage '
                                            'project leaders.')
        return redirect('member_resources:index')
    formset = ManageProjectLeadersFormSet(request.POST or None,
                                          prefix='project_leaders')

    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            request.session['success_message'] = ('Project leaders '
                                                  'successfully added.')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = messages.GENERIC_SUBMIT_ERROR
...
def add_external_service(request):
        ...
        if formset.is_valid():
            ...
            for instance in instances:
                if instance in formset.new_objects:
                    ...
                    instance.event_type = EventCategory.objects.get(
                                                name='External Service Hours')
                prev_ext = ProgressItem.objects.filter(
    ...
    context_dict = {
        ...
        'help_text': ('Note that external service hours only count for '
                      'electees and that a maximum of 5 may be submitted. Any '
                      'hours over 5 for a given electee will be saturated '
                      'at 5.'),
        'can_add_row': True,
    }
    context_dict.update(get_common_context(request))
...
def manage_dues(request):
    ...
    context_dict = {
        ...
        'help_text': ('When electees pay their initition dues click the '
...
def change_requirements(request, distinction_id):
    ...
    if status == 'Active':
        if not Permissions.can_change_active_requirements(request.user):
            request.session['error_message'] = ('You are not authorized to '
                                                ...
                                                'requirements.')
            return redirect('member_resources:index')
    elif (status == 'Electee' and
          standings.filter(name="Undergraduate").exists()):
        if not Permissions.can_change_ugrad_electee_requirements(request.user):
            request.session['error_message'] = ('You are not authorized to '
                                                'change electee requirements.')
            return redirect('member_resources:index')
    ...
    RequirementFormSet = modelformset_factory(Requirement,
    ...
    if request.method == 'POST':
        if formset.is_valid():
            ...
            for instance in instances:
                ...
                instance.save()
            formset.save_m2m()
            request.session['success_message'] = ('Requirements updated '
                                                  'successfuly.')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
...
def change_event_categories(request):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            request.session['success_message'] = ('Event categories '
                                                  'successfully updated')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'help_text': ('For adding, changing, or removing event categories. '
                      'Note that removing an event category will remove any '
                      'requirements, progress items, or events that use that '
                      'event. Really, don\'t do it.'),
        'can_add_row': True,
        }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
...
def view_meeting_feedback_for_term(request, term_id):
    if not Permissions.can_view_meeting_feedback(request.user):
        request.session['error_message'] = ('You are not authorized to view '
                                            'the meeting surveys.')
        return redirect('member_resources:index')
    ...
    if not terms.exists():
        ...
        term = terms[0]
    completed_meetings = CalendarEvent.objects.filter(
                            completed=True,
                            term=term,
                            event_type__name__contains='Meeting Attendance'
    )
    meeting_surveys = MeetingSignInUserData.objects.filter(
                                        meeting_data__event__term=term)
    feedback_surveys = []
    for meeting in completed_meetings:
        feedback_surveys.append({
                        'meeting': meeting,
                        'surveys': MeetingSignInUserData.objects.filter(
                                                meeting_data__event=meeting
                        )
        })
...
def view_meeting_feedback(request):
    return view_meeting_feedback_for_term(
                                    request,
                                    AcademicTerm.get_current_term().id
    )
...
def approve_tutoring_forms(request):
    ...
    formset = TutoringFormFormSet(
                    request.POST or None,
                    queryset=TutoringRecord.objects.filter(approved=False)
    )
    if request.method == 'POST':
        if formset.is_valid():
            instances = formset.save()
            tutoring_category = EventCategory.objects.get(
                                                name__contains='Tutoring')
            for tutoring_record in instances:
                tutoring_credit = ProgressItem(
                                member=tutoring_record.tutor,
                                event_type=tutoring_category,
                                date_completed=tutoring_record.date_tutored,
                                amount_completed=tutoring_record.number_hours,
                                term=AcademicTerm.get_current_term(),
                                name='One-on-One Tutoring'
                )
                tutoring_credit.save()
            request.session['success_message'] = ('Tutoring Forms successfully'
                                                  ' updated')
            return redirect('member_resources:view_misc_reqs')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'help_text': ('These forms are submitted by members after tutoring '
                      'sessions. They must be approved for credit to be '
                      'issued.'),
        'can_add_row': False,
        }
    context_dict.update(get_common_context(request))
...
def view_background_forms(request):
    if not Permissions.can_view_background_forms(request.user):
        request.session['error_message'] = ('You are not authorized to view '
                                            'educational background forms.')
        return redirect('member_resources:index')
    background_forms = EducationalBackgroundForm.objects.all().order_by('id')
    template = loader.get_template(
                        'member_resources/view_background_forms.html')
    context_dict = {
...
def approve_praise(request, praise_id):
    if not hasattr(request.user, 'userprofile'):
        request.session['error_message'] = ('You must create a profile to '
                                            'approve an affirmation.')
        return redirect('member_resources:index')
    tbp = get_object_or_404(TBPraise, id=praise_id)
    if not tbp.public:
        request.session['error_message'] = ('You can only approve public '
                                            'affirmations.')
        return redirect('fora:index')
    if not request.user.userprofile == tbp.recipient:
        request.session['error_message'] = ('You can only approve affirmations'
                                            ' you received.')
        return redirect('fora:index')
    if (tbp.giver == tbp.recipient and tbp.anonymous and tbp.public):
        request.session['error_message'] = ('You can\'t post a self-'
                                            'affirmation anonymously.')
        return redirect('fora:index')
    tbp.approved = True
    tbp.save()
    request.session['success_message'] = ('Affirmation successfully approved '
                                          'for posting')
    return redirect('fora:index')
...
def submit_praise(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            instance.save()
            if not (instance.giver == instance.recipient and
                    instance.anonymous and
                    instance.public):
                instance.email_praise()
            request.session['success_message'] = ('Affirmation submitted '
                                                  'successfully')
            return redirect('fora:index')
        else:
            request.session['error_message'] = INVALID_FORM_MESSAGE
    ...
    context_dict = {
        ...
        'back_button': {
                    ...
                    'text': 'To Member Playground'
        },
        'form_title': 'Affirm/Praise/Congratulate a member',
        'help_text': ('Use this form to give some positive feedback to '
                      'another member (or user). This can be done anonymously '
                      'or attributed to you. You can also indicate that the '
                      'feedback should be private to the recipient or public '
                      '(other members/users can see it). This is only for '
                      '*positive* feedback and should not be used as a means '
                      'to provide anonymous criticism.'),
        }
    context_dict.update(get_common_context(request))
    context_dict.update(get_permissions(request.user))
    return HttpResponse(template.render(context_dict, request))
...
def non_event_project_meta_data(request, ne_report):
    if ne_report:
        NonEventProjectForm = modelform_factory(
                                NonEventProject,
                                form=BaseNEPForm,
                                exclude=('term', 'project_report')
        )
    else:
        NonEventProjectForm = modelform_factory(
                                NonEventProject,
        ...
        ne_report = None
    if ne_report:
        form = NonEventProjectForm(request.POST or None, instance=ne_report)
    else:
        form = NonEventProjectForm(request.POST or None)
    if request.method == 'POST':
        if form.is_valid():
            nep = form.save()
            request.session['success_message'] = ('Non-event project created '
                                                  'successfully')
            return redirect('member_resources:non_event_project_participants',
                            nep.id)
        else:
    ...
    context_dict = {
        ...
        'help_text': ('These are for reports sent to the national '
                      'organization to determine eligibility for certain '
                      'chapter awards. They are also used for transition '
                      'material to help future project leaders perform a '
                      'similar event. Please be descriptive in your '
                      'responses. Note that this form contains the meta-data '
                      'for the project, you\'ll be asked to submit 2 '
    ...
    if request.method == 'POST':
        if formset.is_valid():
            ...
            request.session['error_message'] = ('Form contained errors, was '
                                                'not saved.')
    template = loader.get_template('generic_formset.html')
    form_title = 'Add/Edit Banquet/Honor\'s Brunch Awards for %s' % (
                                                            unicode(term))
    context_dict = {
        'formset': formset,
        'can_add_row': True,
        ...
        'help_text': ('Update the banquet awards for those that received '
                      'them. These show on the recipients\' profiles.'),
        'base': 'member_resources/base_member_resources.html',
        'back_button': {
                'link': reverse('member_resources:access_history'),
                'text': 'To Access History'
        },
    }
    context_dict.update(get_permissions(request.user))
    context_dict.update(get_common_context(request))
...
def manage_awards(request):
    return redirect(
                'member_resources:manage_awards_for_term',
                AcademicTerm.get_current_term().id
    )
...
def non_event_project_participants(request, ne_id):
    ...
    NonEventFormSet = modelformset_factory(
                            NonEventProjectParticipant,
                            form=BaseNEPParticipantForm,
                            exclude=('project',)
    )
    formset = NonEventFormSet(
                    request.POST or None,
                    queryset=NonEventProjectParticipant.objects.filter(
                                        project=ne).order_by(
    ...
    if request.method == 'POST':
        if formset.is_valid():
            ...
            for instance in instances:
                instance.project = ne
                instance.save()
            request.session['success_message'] = ('Non-event project '
    ...
    context_dict = {
        ...
        'help_text': ('These are for reports sent to the national organization'
                      ...
                      'be descriptive in your responses. Note that this form '
                      'contains the participants for the project, you\'ll be '
                      'asked to submit 1 additional form with it.'),
        'base': 'member_resources/base_member_resources.html',
    }
    context_dict.update(get_permissions(request.user))
    context_dict.update(get_common_context(request))
...
def view_electee_surveys_for_term(request, term_id):
    ...
    if profile.status.name == 'Electee':
        ...
        user_visibilities.add('A')
    term = get_object_or_404(AcademicTerm, id=term_id)
    interview_shifts = InterviewShift.objects.filter(
                            interviewer_shift__attendees__in=[profile])
    ...
    if current_survey:
        ...
        answers = SurveyAnswer.objects.filter(question__in=questions,
                                              term=term).distinct()
        parts = SurveyPart.objects.filter(
                                surveyquestion__in=questions).distinct()
    ...
    for part in sorted(parts):
        output_table += "<th colspan=\"%d\">%s</th>" % (
                            questions.filter(part=part).distinct().count(),
                            my_markdown(unicode(part)))
    output_table += "</tr><tr><th></th>"
    for part in sorted(parts):
        for question in questions.filter(
                            part=part).distinct().order_by('display_order'):
            output_table += "<th>%s</th>" % (my_markdown(question.text))
    output_table += "</tr></thead><tbody>"
    for electee in electees:
        output_table += "<tr><td><a href=\"%s\">%s</a></td>" % (
                            reverse(
                                'member_resources:view_electee_survey',
                                args=[electee.uniqname]
                            ),
                            unicode(electee))
        for part in sorted(parts):
            for question in questions.filter(
                            part=part).distinct().order_by('display_order'):
                electee_answers = answers.filter(submitter=electee,
                                                 question=question)
                if (part.visibility in user_visibilities or
                   electee == profile or
                   interview_shifts.filter(
                        interviewee_shift__attendees__in=[electee]).exists()):
                    if electee_answers.exists():
                        output_table += "<td>%s</td>" % my_markdown(
                                                    electee_answers[0].answer)
                    else:
    ...
    output_table += "</tbody>"
    template = loader.get_template(
                            'member_resources/view_electee_surveys.html')
    context_dict = {
        'table': output_table,
        'subnav': 'history',
        'base': 'member_resources/base_member_resources.html',
...
def view_electee_surveys(request):
    return redirect('member_resources:view_electee_surveys_for_term',
                    AcademicTerm.get_current_term().id)
...
def add_background_checks(request):
    if not Permissions.can_manage_background_checks(request.user):
        ...
        return get_previous_page(request, alternate='member_resources:index')
    prefix = 'background'
    BackgroundForm = modelformset_factory(BackgroundCheck,
                                          form=BaseBackgroundCheckForm)
    formset = BackgroundForm(
                    request.POST or None,
                    queryset=BackgroundCheck.objects.none(),
                    prefix=prefix
    )
    if request.method == 'POST':
        if formset.is_valid():
            formset.save()
            request.session['success_message'] = ('Background check statuses '
                                                  'added successfully')
            return get_previous_page(request,
                                     alternate='member_resources:index')
        else:
            request.session['error_message'] = ('There was an error in your '
                                                'submission, please correct '
                                                'it')
    template = loader.get_template('generic_formset.html')
    context_dict = {
        'formset': formset,
        ...
        'help_text': ('These are stored to verify that members who attend '
                      'events with minors have undergone the appropriate '
                      'screening. They remain valid for a set period of time '
                      'and then expire. Thus these should only be added when '
                      'updated, not semesterly.'),
        'base': 'member_resources/base_member_resources.html',
    }
    context_dict.update(get_permissions(request.user))
    context_dict.update(get_common_context(request))
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if not bad_uniqnames:
                request.session['success_message'] = ('Background check '
                                                      'statuses added '
                                                      'successfully')
                return get_previous_page(request,
                                         alternate='member_resources:index')
            else:
                bad_uniq_list = ', '.join(bad_uniqnames)
                warn_message = 'The following uniqnames do not have profiles: '
                warn_message = warn_message + bad_uniq_list
                request.session['success_message'] = ('Form was valid, some '
                                                      'uniqnames added')
                request.session['warning_message'] = warn_message
                form = MassAddBackgroundCheckForm(
                            initial={
                                'uniqnames': '\n'.join(bad_uniqnames),
        ...
        for question in questions.filter(
            ...
            if (part.visibility in user_visibilities or
               ...
               interview_shifts.filter(
                    interviewee_shift__attendees__in=[electee]).exists()):
                if electee_answers.exists():
                    part_data['qas'].append({'q': question,
                                             'a': electee_answers[0].answer})
                else:
                    part_data['qas'].append({'q': question,
                                             'a': 'No answer given'})
            else:
...
def view_photos(request):
    if not Permissions.can_create_events(request.user):
        request.session['error_message'] = ('You are not authorized '
                                            'to access photos')
        return get_previous_page(request, alternate='member_resources:index')
    context_dict = {
