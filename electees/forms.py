...
from django.forms.models import inlineformset_factory, BaseInlineFormSet,BaseModelFormSet,modelformset_factory
...
class ManualElecteeGroupMembersForm(forms.ModelForm):
    ...
    group_name=forms.CharField(widget=forms.TextInput(attrs={'class':'disabled','readonly':'readonly'}))
    
    members_bulk = forms.CharField(label='Member uniqnames (one per line)',widget=forms.Textarea,required=False)
    def clean(self):
        ...
        for uniqname in members_bulk.split('\n'):
            ...
            try:
                ...
                if member.electee_group_members.count():
                    errors.append(ValidationError(_('%(uniqname)s already in an electee team'),code='duplicate',params={'uniqname':stripped_uniqname}))
                if not member.status.name =='Electee':
                    errors.append(ValidationError(_('%(uniqname)s is not an electee'),code='invalid',params={'uniqname':stripped_uniqname}))
            except ObjectDoesNotExist:
                errors.append(ValidationError(_('%(uniqname)s is not a member'),code='invalid',params={'uniqname':stripped_uniqname}))
        if errors:
    ...
    def save(self,commit=True):
        ...
        for uniqname in members_bulk.split('\n'):
            try:
                member = MemberProfile.objects.get(uniqname=uniqname.lstrip().rstrip())
        ...
        for form in self.forms:
            ...
            for uniqname in group_uniqnames:
                ...
                if stripped_uniqname in uniqnames:
                    errors.append(ValidationError(_('%(uniqname)s can only be added to one team'),code='duplicate',params={'uniqname':stripped_uniqname}))
                else:
                    uniqnames.add(stripped_uniqname)
        if errors:
            raise ValidationError(errors)
ManualElecteeGroupMembersFormSet = modelformset_factory(ElecteeGroup,form=ManualElecteeGroupMembersForm, exclude = ['leaders','officers','points','term','members'],extra=0, formset=BaseManualElecteeGroupMembersFormSet)
