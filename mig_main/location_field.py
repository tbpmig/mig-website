    ...
    def from_string(cls, string):
        ...
        if len(ll) != 2:
            raise ValidationError(
                        _('Location must be a latitude/longitude pair.')
                  )
        try:
...
class MapWidget(forms.TextInput):
    class Media:
        js = (('https://maps.googleapis.com/maps/api/'
               'js?key=AIzaSyBcW7EQwfPZa3poz2Hr3MPllP4vjpilcLc'),)
...
class LocationField(models.CharField):
    ...
    def to_python(self, value):
        ...
        if len(ll) != 2:
            raise ValidationError(
                    "Invalid input for GeoLocation instance %s" % unicode(ll)
            )
        latitude = Decimal(ll[0].strip())
        longitude = Decimal(ll[1].strip())
        return GeoLocation(latitude, longitude)
